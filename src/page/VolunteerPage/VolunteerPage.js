...
function VolunteerPage() {
  ...
  const hanndleRegister = async () => {
    ...
    console.log(moment(formData.date_of_birth).diff(new Date(), "years"));
    if (moment(formData.date_of_birth).diff(new Date(), "years") * -1 <= 18) {
      setShowError(true);
      toast.error("Chưa đủ 18!");
      return;
    }
  ...
  return (
    <div className={"relative"}>
      ...
      <div
        className={
          " flex md:flex-row flex-col justify-center py-10 md:px-0 px-10 gap-10 md:w-4/5 mx-auto w-full"
        }
      >
        <div className="h-full flex flex-column justify-center md:w-1/2 w-full">
          <h2 className="text-[45px] font-['Calistoga'] text-black mb-[41px]">
            TÌNH NGUYỆN VIÊN
          </h2>
          <span className="text-[24px] text-black">
            Vui lòng điền vào thông tin bên cạnh nếu các bạn muốn trở thành tình
            nguyện viên trong các chiến dịch tình nguyện sắp tới của chúng tôi.
            Sau khi đăng ký chúng tôi sẽ liên hệ với bạn trong thời gian sớm
            nhất.
          </span>
        </div>
